...
@Slf4j
public class EventBatchServiceRedis {
	private final EventRedisBuffer eventRedisBuffer; // 변경: EventBuffer -> EventRedisBuffer
	...
	public void processBatchForSession(SessionDataKey sessionDataKey, boolean isInactive) {
		log.info("{} 세션 이벤트 일괄 처리 시작 (Redis)", sessionDataKey);
	...
	private int countPendingEvents(SessionDataKey sessionDataKey) {
		return eventRedisBuffer.getClickEvents(sessionDataKey).size() // 변경
			+ eventRedisBuffer.getMoveEvents(sessionDataKey).size()   // 변경
			+ eventRedisBuffer.getScrollEvents(sessionDataKey).size(); // 변경
	}
	...
	private void processClickEvents(SessionDataKey sessionDataKey, SessionData sessionData) {
		List<PointerClickEventRequest> clickEvents = eventRedisBuffer.flushClickEvents(sessionDataKey); // 변경
		log.info("세션 {}: 클릭 이벤트 {} 개 처리 (Redis)", sessionDataKey, clickEvents.size());
	...
	private void processMoveEvents(SessionDataKey sessionDataKey, SessionData sessionData) {
		List<PointerMoveEventRequest> moveEvents = eventRedisBuffer.flushMoveEvents(sessionDataKey); // 변경
		log.info("세션 {}: 이동 이벤트 {} 개 처리 (Redis)", sessionDataKey, moveEvents.size());
	...
	private void processScrollEvents(SessionDataKey sessionDataKey, SessionData sessionData) {
		List<PointerScrollEventRequest> scrollEvents = eventRedisBuffer.flushScrollEvents(sessionDataKey); // 변경
		log.info("세션 {}: 스크롤 이벤트 {} 개 처리 (Redis)", sessionDataKey, scrollEvents.size());
